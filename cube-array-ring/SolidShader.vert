...
//    gl_Position = mvpMatrix * vec4(sinDist * sinTime2 * vertexPos + vertexCenter, 1);
    ...
    sinTime2 = (sin(5.0f * (time - ringCenter.y)) / 2.0f + 0.5f) + 0.5f;
    sinTime3 = (sin(5.0f * (time - ringCenter.y) + 3.14159f/2) / 2.0f + 0.5f) + 0.5f;
    // sets the vertex position to whatever was in the buffer
    gl_Position = vpMatrix * vec4((sinTime2 + sinTime3) * vertexPos
                                  + ringCenter + ringOffset, 1);
