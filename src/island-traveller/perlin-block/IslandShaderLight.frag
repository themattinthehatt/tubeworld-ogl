...
// PerlinBlockIsland class handles this data
struct Material {
...
// PerlinBlockLight class handles this data
struct DirLight {
...
vec3 calcDirLight(vec3 normal, vec3 viewDir) {
    ...
    vec3 specular = dirLight.specular * spec * material.specular; // full spec highlights for now
...
vec3 calcPointLight(PointLight light, vec3 normalVec, vec3 viewDir, vec3 fragPos) {
    ...
    // calculate ambient lighting
    vec3 ambient = light.ambient * vec3(texture(material.loadedTexture, texCoords));

    // calculate diffuse lighting
    vec3 lightDir = normalize(light.position - fragPos);
    float diff = max(dot(normalVec, lightDir), 0.0);
    vec3 diffuse = light.diffuse * diff * vec3(texture(material.loadedTexture, texCoords));
